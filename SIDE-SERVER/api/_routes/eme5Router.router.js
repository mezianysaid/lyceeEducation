...
const lessons = require('../_models/eme5Lessons.model');
...
router.post('/addBlog', async(req, res) => {
    cours = await lessons.findById({ _id: req.body.cours })
...
router.get('/listLissons/:categ', async(req, res) => {
    try {
        courses = await lessons.find({ module: req.params.categ }).populate({ path: 'blogs', populate: { path: 'images' } }).populate({ path: 'tds', populate: { path: 'images' } }).populate('pdfs');
...
router.post('/addPdf', storage, async(req, res) => {
    let lesson = await lessons.findOne({ _id: req.body.id }).exec()
    let pdf = new pdfs({
...
router.get('/Search/:title', async(req, res) => {
    try {
        const lessn = await (await lessons.find({ title: req.params.title }).populate({ path: 'blogs', populate: { path: 'images' } }).populate({ path: 'tds', populate: { path: 'images' } }).populate('pdfs'));
        console.log(lessn)
        return res.send(lessn)
    } catch (error) {
