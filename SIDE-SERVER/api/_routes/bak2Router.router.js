...
const lessons = require('../_models/bak2Lessons');
...
router.get('/Search/:title', async(req, res) => {
    try {
        const lssns = await (await lessons.find({ title: req.params.title }).populate({ path: 'blogs', populate: { path: 'images' } }).populate({ path: 'tds', populate: { path: 'images' } }).populate('pdfs'));
        return res.send(lssns)
